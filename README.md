...
- **Fill form** (Pillar 5): voice-guided field-by-field fill with a "sounds right? / try again" check per field, or **snap-to-form auto-fill** — photograph an Aadhaar card, ration card, PAN or mark sheet and the form populates itself. Two deliberately separate extraction passes:
  - **On-device regex** (`src/lib/documentFields.ts`) for document type, ID number and date of birth. Government ID numbers never leave the browser, and Aadhaar is **masked to its last 4 digits** (`XXXX XXXX 0123`) the way real KYC forms and UIDAI guidance handle it — someone reading a low-vision user's screen aloud shouldn't be reading out a full Aadhaar number.
  - **LLM** (`src/lib/extractFields.ts`) for names and addresses, which have no reliable pattern. The text is redacted first, so the ID number and DOB are already stripped before anything is sent off-device.
  - ID number is intentionally *not* in the spoken walkthrough — asking someone to say their Aadhaar number aloud, likely in a queue or with a helper present, is the wrong way to collect it.
